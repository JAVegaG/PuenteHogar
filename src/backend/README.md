...
└── modules/
    ...
    ├── rental-tracking/        # Máquina de estados del arriendo (dominio, aplicación, infraestructura, controlador), resolución listing→lease para transiciones desde el frontend, autorización relajada para CONTACT_INITIATED (cualquier tenant autenticado) y para requestingUserId='system' (transiciones automatizadas desde webhooks), notificaciones in-app (CONTACT_INITIATED con metadata de contacto del arrendatario, CONTRACT_SIGNED, PAYMENT_RECEIVED) via TrackingNotificationAdapter
