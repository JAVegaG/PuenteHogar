...
└── modules/
    ...
    ├── landlord-portfolio/     # Portafolios (CRUD completo: crear, listar, actualizar, eliminar), unidades enriquecidas (cross-schema Property+Address+PortfolioUnit, con unitStatus/hasActiveListing/tenantName/monthlyRent computados; crear, actualizar, eliminar con validación de arriendos activos), leases (listado por unidad, detalle con info de arrendatario descifrada via IPIIEncryptor, cancelación basada en contractStatus — permite cancelar si contrato no está firmado), tenant-leases controller (GET /leases/:leaseId/detail — detalle de arriendo para arrendatario con info de inmueble, canon mensual y próximo pago via cross-module query a payments), catálogo geográfico (departamentos/ciudades DANE), notificaciones in-app (LEASE_CREATED, LEASE_CANCELLED) via PortfolioNotificationAdapter, derivación de unitStatus "Ocupado" requiere trackingStatus ∈ {CONTRACT_SIGNED, PAYMENT_RECEIVED} (leases en estados pre-firma se tratan como "Disponible")
    ├── contracts/              # Contratos (CRUD: crear con upload S3 real, consultar con presigned URL, reemplazar PDF en PENDING, eliminar con guardas de estado), firma electrónica, almacenamiento de documentos en S3 (presigned URLs, 15 min TTL), listado de contratos por arrendador (cross-schema: Contract→Lease→PortfolioUnit→LandlordPortfolio), listado de contratos por arrendatario (cross-schema: ContractParty→Contract→Lease→PortfolioUnit, con resolución de nombre de unidad y arrendador via PII decryption), signing details por parte, notificaciones in-app (CONTRACT_SIGNED, CONTRACT_UPLOADED, signing_failed) via ContractNotificationAdapter, programación de pago inicial al completar firma (fire-and-forget via IPaymentSchedulingPort → PaymentSchedulingAdapter en payments), transición de estado de tracking al subir contrato (→CONTRACT_UPLOADED) y al completar firma (→CONTRACT_SIGNED) via TransitionLeaseStateUseCase de rental-tracking, desactivación automática de listing al firmar contrato (fire-and-forget via IListingDeactivationPort → ListingDeactivationAdapter)
    ├── payments/               # Pagos, pasarela, idempotencia (dominio, aplicación, infraestructura, controlador), notificaciones in-app (PAYMENT_RECEIVED) via PaymentNotificationAdapter, PaymentSchedulingAdapter (implementa IPaymentSchedulingPort del módulo contracts — crea ScheduledPayment en DB), endpoints tenant-facing: GET /payments/units (tarjetas agrupadas por unidad), GET /payments/units/:unitId/history (historial paginado con filtro por estado), GET /payments/:paymentId/detail (detalle con desglose de ítems y recibo para pagos realizados)
    ├── accounting/             # Reportes financieros (dominio, aplicación, infraestructura: PrismaAccountingRepository + RedisReportCache)
    ├── rental-tracking/        # Máquina de estados del arriendo (dominio, aplicación, infraestructura, controlador), resolución listing→lease para transiciones desde el frontend (con creación automática de lease si no existe para la unidad del listing), autorización relajada para CONTACT_INITIATED (cualquier tenant autenticado) y para requestingUserId='system' (transiciones automatizadas desde webhooks), notificaciones in-app (CONTACT_INITIATED con metadata de contacto del arrendatario, CONTRACT_SIGNED, PAYMENT_RECEIVED) via TrackingNotificationAdapter
    ...
    └── shared/                 # Helpers compartidos entre módulos de dominio (ETL round-trip property-based tests)
```
...
| `PrismaService` | Cliente Prisma compartido entre módulos. Usa `PrismaPg` adapter (`@prisma/adapter-pg`) para la conexión. Construye `DATABASE_URL` internamente desde variables individuales (`DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`) con `?sslmode=no-verify` para conexiones TLS a RDS (acepta certificados auto-firmados), o usa `DATABASE_URL` si ya está definida. Ejecuta `prisma migrate deploy` programáticamente en `onModuleInit` antes de conectar. Un guard estático (`private static initialized`) garantiza que migraciones y seeds se ejecuten **una sola vez** aunque múltiples módulos instancien `PrismaService`. Los seeds de catálogo se ejecutan de forma **asíncrona en background** (via `setImmediate`) después de que la app está lista para servir tráfico — esto evita que el health check del ALB falle por timeout mientras los seeds corren. (`@src/shared/prisma/`) |
...
`RentalTrackingModule` exporta `TransitionLeaseStateUseCase` — los módulos que necesiten avanzar el estado del ciclo de vida del arriendo (e.g. `ContractsModule`) importan `RentalTrackingModule` e inyectan `TransitionLeaseStateUseCase` directamente. Las transiciones se ejecutan fire-and-forget (`.catch(() => undefined)`) para no bloquear la operación principal.
...
Ports donde un módulo define la interfaz y otro módulo provee la implementación:

| Token | Módulo que define | Módulo que implementa | Adaptador | Descripción |
|-------|-------------------|----------------------|-----------|-------------|
| `PAYMENT_SCHEDULING_PORT` | `contracts` | `payments` | `PaymentSchedulingAdapter` | Crea `ScheduledPayment` al completarse la firma de un contrato (fire-and-forget) |
| `LISTING_DEACTIVATION_PORT` | `contracts` | `contracts` (local) | `ListingDeactivationAdapter` | Desactiva el listing activo de la unidad al completarse la firma del contrato (fire-and-forget). Resuelve lease → portfolio_unit_id → listing y marca `is_active = false` |
...
| `PAYMENTS_CROSS_MODULE_QUERY` | `payments` | `hasPendingPayments(userId)`, `getNextPendingPayment(leaseId)` |
