...
## 2. NetworkStack
- [ ] 2.1 Update `lib/stacks/network-stack.ts` — rename VPC Connector SG to ECS Service SG, add ALB security group, allow ECS SG outbound to internet (for ECR pulls)
- [ ] 2.2 Create ECS service security group (outbound to data SG on ports 5432, 6379; outbound to internet via NAT for ECR pulls)
- [ ] 2.3 Create data security group (inbound from ECS service SG on ports 5432, 6379 only)
- [ ] 2.4 Create ALB security group (inbound 80/443 from 0.0.0.0/0, outbound to ECS service SG on port 3000)
...
- [ ] 2.8 Export all outputs (vpc, subnets, security groups including ALB SG) as stack properties for cross-stack references
...
- [ ] 5.1 Rewrite `lib/stacks/compute-stack.ts` to use ECS Fargate instead of App Runner, accepting Network, Data, and Ci stack outputs as props
- [ ] 5.2 Create ECS Cluster
- [ ] 5.3 Create Application Load Balancer (ALB) in public subnets with HTTP→HTTPS redirect and path-based routing
- [ ] 5.4 Create backend target group with health check at `/api/health`
- [ ] 5.5 Create frontend target group with health check at `/`
- [ ] 5.6 Create ALB listener rules: `/api/*` → backend target group, default → frontend target group
- [ ] 5.7 Create IAM task execution role (ECR pull, Secrets Manager read for secret injection, CloudWatch logs)
- [ ] 5.8 Create IAM task role for backend (Secrets Manager read on specific ARNs, S3 read/write on assets bucket, CloudWatch logs)
- [ ] 5.9 Create IAM task role for frontend (CloudWatch logs only — no S3, no Secrets Manager)
- [ ] 5.10 Create backend ECS task definition with: ECR image, environment variables from CDK cross-stack refs, secrets from Secrets Manager ARNs, CPU/memory per environment
- [ ] 5.11 Create frontend ECS task definition with: ECR image, environment variables (NEXT_PUBLIC_API_URL from ALB DNS), CPU/memory per environment
- [ ] 5.12 Create backend ECS Fargate service in private subnets with ALB target group, desired count per environment
- [ ] 5.13 Create frontend ECS Fargate service in private subnets with ALB target group, desired count per environment
- [ ] 5.14 Configure ECS Service Auto Scaling (target tracking on CPU utilization 70%) per environment
- [ ] 5.15 Export ALB DNS name, ALB ARN, and service ARNs as stack outputs
...
- [ ] 6.1 Update `lib/stacks/cdn-stack.ts` to use ALB as origin instead of App Runner URLs
- [ ] 6.2 Create WAF Web ACL with: rate limiting (2000 req/5min/IP), AWS Common Rule Set, Known Bad Inputs, SQLi protection, XSS protection
- [ ] 6.3 Create CloudFront distribution with two origins: ALB (for API and frontend), S3 bucket (OAC for /assets/*)
- [ ] 6.4 Configure cache behaviors: `/api/*` → ALB (no cache), `/assets/*` → S3 (1-year cache), default → ALB (no cache, frontend SSR)
- [ ] 6.5 Enable compression (gzip + Brotli), set minimum TLS 1.2, enable HTTP/2
- [ ] 6.6 Conditionally create ACM certificate and attach to distribution if `domainName` is provided in config
- [ ] 6.7 Update S3 bucket policy to allow access only from CloudFront OAC and backend task role
...
## 7. MonitoringStack
- [ ] 7.1 Update `lib/stacks/monitoring-stack.ts` to use ECS/ALB metrics instead of App Runner metrics
- [ ] 7.2 Create SNS topic for alarm notifications with email subscription parameter
- [ ] 7.3 Create CloudWatch alarms: ALB target response time > 800ms (p95), 5xx error rate > 1%, RDS CPU > 80%, RDS free storage < 20%
- [ ] 7.4 Create CloudWatch log groups with retention: 30 days (staging), 90 days (production)
- [ ] 7.5 Create CloudWatch dashboard with key metrics: ALB request count, target response time percentiles, HTTP error rates, ECS CPU/memory utilization, DB connections
...
- [ ] 9.2 Update `test/data-stack.test.ts` — verify RDS encryption, Multi-AZ (production), backup retention, S3 encryption and public access block
- [ ] 9.3 Rewrite `test/compute-stack.test.ts` — verify ECS cluster, ALB, Fargate services, task definitions, IAM role policies (least privilege), auto-scaling config
- [ ] 9.4 Update `test/cdn-stack.test.ts` — verify CloudFront origins (ALB + S3), cache behaviors, WAF attachment, TLS minimum version
- [ ] 9.5 Update `test/monitoring-stack.test.ts` — verify alarms exist with correct thresholds (using ALB/ECS metrics)
- [ ] 9.6 Update snapshot tests for all stacks to detect unintended changes
- [ ] 9.7 Run full test suite: `npm test` — all tests must pass

**Checkpoint**: All CDK tests pass (`npm test` exits with code 0).

## 10. Documentation
- [ ] 10.1 Update `src/infra/README.md` to reflect ECS Fargate architecture (replace all App Runner references)
...
## 11. Final Validation
- [ ] 11.1 Run `npx cdk synth --all -c env=staging` — verify all stacks synthesize without errors
- [ ] 11.2 Run `npx cdk synth --all -c env=production` — verify production config differences (Multi-AZ, desiredCount: 2, larger task definitions)
- [ ] 11.3 Run `npm test` — all CDK assertion and snapshot tests pass
