...
El sistema actual expone un API plano de unidades (`GET /portfolio/units`, `POST /portfolio/units`, `PATCH /portfolio/units/:id`) sin agrupación por portafolio, sin datos agregados, y con un formulario de creación de unidades limitado a `propertyId`, `leaseBaseAmount`, `leaseBaseCurrency` y `conditions`. Los diseños de Figma requieren una reestructuración tanto del backend como del frontend para soportar la vista de portafolios con estadísticas, la creación de portafolios, y un formulario de unidades con campos como nombre/identificación, dirección, área (largo × ancho), habitaciones, baños, canon base y descripción.
