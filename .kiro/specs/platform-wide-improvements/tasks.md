...
- [ ] 1. Soft delete — Prisma schema migration and middleware
  - [x] 1.1 Add `deleted_at DateTime?` column to every Prisma model
  ...
  - [x] 1.2 Implement Prisma soft-delete utilities
    - Create `src/backend/src/shared/prisma/soft-delete.utils.ts`
    - Intercept `findMany`, `findFirst`, `findUnique`, `count` to inject `where: { deleted_at: null }` via `softDeleteFilter` / `withSoftDeleteFilter` helpers
    - Provide `softDeleteData()` helper to convert `delete` → `update` with `deleted_at: new Date()`
    - Skip middleware for models without `deleted_at` (RAW tables)
    - Support bypass: when `deleted_at` is explicitly passed, `withSoftDeleteFilter` does not inject the filter
    - Utilities are imported by repositories as needed (Prisma 6+ does not support `$use` middleware)
...
- [ ] 2. RAW/ETL hybrid persistence fix
  - [x] 2.1 Fix `JSON.stringify` in users module RAW persistence
  ...
  - [x] 2.2 Fix `JSON.stringify` in landlord-portfolio module RAW persistence (if present)
  ...
  - [x] 2.3 Create `parsePayload` ETL helper for backward compatibility
...
- [ ] 4. Eliminate cross-schema direct database queries
  - [x] 4.1 Create `IPortfolioCrossModuleQuery` port and implementation
  ...
  - [x] 4.2 Create `IContractsCrossModuleQuery` port and implementation
  ...
  - [x] 4.3 Create `IPaymentsCrossModuleQuery` port and implementation
  ...
  - [x] 4.4 Refactor `PrismaUserRepository` to use cross-module ports
...
- [ ] 6. Auth pages navigation — hamburger menu instead of back button
  - [x] 6.1 Update login page to use hamburger menu
  ...
  - [x] 6.2 Update registration page to use hamburger menu
...
- [ ] 7. Income page — show portfolio units even when income is zero
  - [x] 7.1 Ensure income detail page renders all units regardless of income
  ...
  - [x] 7.2 Verify `PortfolioIncomeCard` always displays `totalUnits`
...
- [ ] 8. Notification type translations and CTA button style
  - [x] 8.1 Add missing notification type translations and fallback formatter
  ...
  - [x] 8.3 Restyle "Gestionar preferencias" CTA as primary button
...
- [ ] 9. Desktop container consistency — `max-w-[560px]` centered layout
  - [x] 9.1 Update auth pages container from `max-w-[448px]` to `max-w-[560px]`
  ...
  - [x] 9.2 Add centered container to notification pages
  ...
  - [x] 9.3 Audit and fix remaining pages for centered container
...
- [x] 10. Checkpoint — Ensure all frontend and backend tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 11. Documentation and steering file updates
  - [x] 11.1 Update steering files with new backend standards
  ...
  - [x] 11.2 Update steering files with new frontend standards
  ...
  - [x] 11.3 Update SRS and architectural design documents
